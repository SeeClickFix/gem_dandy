require 'depbot/gem_change'

RSpec.describe Depbot::GemChange do
  let(:name) { 'depbot' }
  let(:previous_version) { '0.0.0' }
  let(:current_version) { '0.0.1' }

  let(:github_url) { 'https://github.com/seeclickfix/depbot' }
  ...
  describe '#github_url' do
    ...
    context 'with a homepage_url and source_code_url not on github' do
      let(:non_github_url) { 'http://bitbucket.com/seeclickfix/depbot' }
  ...
  describe '#changelog_url' do
    it 'delegates to Github::Changelog.for' do
      ...
      allow(Depbot::Github::Changelog).to receive(:for)
        .and_return(changelog_url)

      expect(subject.changelog_url).to eq(changelog_url)
      expect(Depbot::Github::Changelog).to have_received(:for)
        .with('seeclickfix/depbot', 'v0.0.1')
    end
  end
